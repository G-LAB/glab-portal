...
class GoogleAPI
{
  ...
  function __construct($consumer_key_or_config, $consumer_secret=null, $api_key=null)
  {
    if (is_array($consumer_key_or_config) === true)
    {
      ...
      $this->api_key = element('api_key', $consumer_key_or_config);
    }
    else
    {
  ...
  function request($apps_email, $method, $url, $params=array())
  {
    $consumer = new OAuthConsumer($this->consumer_key, $this->consumer_secret, NULL);

    $params = array(
      'xoauth_requestor_id' => $apps_email,
      'key'=>'AIzaSyDzSQUfOcOun-ifriJ92Vqaf79L-VYxstI'
    );

    $request = OAuthRequest::from_consumer_and_token($consumer, NULL, strtoupper($method), $url, $params);
    ...
    return send_request($request->get_normalized_http_method(), $url, $request->to_header());
  }
